...
class DateTimeFunctions:
    ...
    @staticmethod
    def getSecondsFromNow(normalizedDateTime: datetime.datetime, timezone: str) -> int:
        now = DateTimeFunctions.getDateTimeNowNormalized(timezone)
        return round((normalizedDateTime - now).total_seconds())

    @staticmethod
    def getMinutesFromNow(normalizedDateTime: datetime.datetime, timezone: str) -> int:
        return round(DateTimeFunctions.getSecondsFromNow(normalizedDateTime, timezone) / 60)

    @staticmethod
    def getHoursFromNow(normalizedDateTime: datetime.datetime, timezone: str) -> int:
        return round(DateTimeFunctions.getSecondsFromNow(normalizedDateTime, timezone) / (60*60))
